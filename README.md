# plactica-html

<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <style>
        body {
            background-color: rgb(0, 0, 0);
            color: white; /* Esto es opcional si quieres que el texto sea visible sobre el fondo negro */
        }
    </style>
</head>
<body>

</body>
</html>

<!DOCTYPE html>
<html>
<head>
<title>Page Title</title>

</head>
<body>

<h1>7 PALABRAS ESENCIA___________VIVA CRITO REEEEYYYYY</h1> 
<img src="Broly-GIF-4k.gif" width="500" height="250"> <img src="goku-ultra-instinct.gif" width="500" height="250">
<p>TE VOY A GOLPEAR HASTA QUE SIENTAS LA ESENCIA___________________me las pagaraaas</p>

<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Transición con Imágenes</title>
    <style>
        .image-container {
            width: 300px;
            height: 200px;
            background-image: url('08.gif'); /* Imagen inicial */descarga (3).jpg
            transition: background-image 4s ease-in-out; /* Duración de la transición */
        }

        .image-container:hover {
            background-image: url('tenor.gif'); /* Imagen al pasar el mouse */
    </style>
</head>
<body>
    <div class="image-container"></div>
</body>
</html><!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Transición con Imágenes</title>
    <style>
        body {
            background-color: rgb(0, 0, 0);
            color: white;
        }
        .image-container {
            width: 300px;
            height: 200px;
            background-image: url('08.gif');
            transition: background-image 4s ease-in-out;
        }
        .image-container:hover {
            background-image: url('tenor.gif');
        }
    </style>
</head>
<body>
    <h1>7 PALABRAS ESENCIALES</h1>
    <img src="Broly-GIF-4k.gif" width="500" height="250">
    <img src="goku-ultra-instinct.gif" width="500" height="250">
    <p>TE VOY A GOLPEAR HASTA QUE SIENTAS LA ESENCIA</p>
    <div class="image-container"></div>
</body>
</html>

ejecucionj

![image](https://github.com/user-attachments/assets/b8bb723f-2ea5-4f4d-9347-377621621fcd)
